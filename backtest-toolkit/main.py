"""
Buy-the-dip strategy backtest and comparison tool.

Pulls real daily (or intraday) price data for a ticker (falling back to
clearly-labeled synthetic data if there's no network access), splits it
into a train period (used only to fit the optional ML dip-filter and
calibrate its threshold) and a held-out test period, then compares five
strategies on the test period:
  ...
  4. Day-trading dip buy (fixed profit target / stop-loss instead of a
     recovery exit)
  5. Bollinger Band breakout (a trend-following strategy, the opposite
     bet from the four dip-buying strategies above)

This is a research/backtesting tool. It does not place real trades and is
not investment advice. See README.md before ever pointing any strategy
built with this toolkit at a live brokerage account.
"""
...
def main():
    ...
    parser.add_argument("--cost-bps", type=float, default=5.0,
                         help="cost in basis points charged on EACH position change (so a full "
                              ...
                              "stock default when backtesting day-trading on crypto")
