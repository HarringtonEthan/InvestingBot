...
# InvestingBot — Version Richards 0.15.0
...
numbers. See `CHANGELOG.md` 0.11.0 for the original build (which
replaced `results/trade_dashboard.png` as the primary view), 0.11.1/0.11.2
for fixes from real usage, 0.12.0 for the full professional redesign
(tabs, colour scheme, a real bug where a data render error could
silently kill every tab's click handler), and 0.14.0 for the animated
background/glass-card visual pass that replaced the site's one earlier
playful touch (a panda intro splash).
